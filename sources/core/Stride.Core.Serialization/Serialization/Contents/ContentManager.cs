...
public sealed partial class ContentManager : IContentManager
{
    ...
    {
        if (settings == null) settings = ContentManagerLoaderSettings.Default;
    ...
    public bool Reload(object obj, string newUrl = null, ContentManagerLoaderSettings settings = null)
    {
        ...
        {
            Reference reference;
            if (!LoadedAssetReferences.TryGetValue(obj, out reference)) return false; // The object is not loaded
    ...
    public void Unload(object obj)
    {
        ...
        {
            Reference reference;
            if (!LoadedAssetReferences.TryGetValue(obj, out reference)) throw new InvalidOperationException("Content not loaded through this ContentManager.");
    ...
    public void Unload(string url)
    {
        lock (LoadedAssetUrls)
        {
            ...
            Reference reference;
            if (!LoadedAssetUrls.TryGetValue(url, out reference)) throw new InvalidOperationException("Content not loaded through this ContentManager.");
    ...
    private object DeserializeObject(Queue<DeserializeOperation> serializeOperations, Reference parentReference, string url, Type objType, object obj, ContentManagerLoaderSettings settings)
    {
        ...
        try
        {
            using (var stream = FileProvider.OpenStream(url, VirtualFileMode.Open, VirtualFileAccess.Read))
            {
                ...
                if (serializer == null) throw new InvalidOperationException($"Content serializer for {url} could not be found. Was expecting to find type {objType} but the actual type of the resource was {headerObjType}");
                contentSerializerContext = new ContentSerializerContext(url, ArchiveMode.Deserialize, this) { LoadContentReferences = settings.LoadContentReferences, AllowContentStreaming = settings.AllowContentStreaming, };
    ...
    private void SerializeObject(Queue<SerializeOperation> serializeOperations, string url, object obj, bool publicReference, Type storageType = null)
    {
        ...
        if (LoadedAssetReferences.ContainsKey(obj)) return;

        var serializer = Serializer.GetSerializer(storageType, obj.GetType());
        if (serializer == null) throw new InvalidOperationException($"Content serializer for {obj.GetType()} could not be found.");
        ...
        foreach (var contentReference in contentSerializerContext.ContentReferences)
        {
            if (contentReference.ObjectValue != null)
            {
                ...
                if (attachedReference == null || attachedReference.IsProxy) continue;
    ...
    internal void SetAssetObject(Reference reference, object obj)
    {
        ...
        {
            if (reference.Object != obj) throw new InvalidOperationException("SetAssetObject has already been called with a different object");
        ...
        lock (LoadedAssetUrls)
        {
            ...
            if (LoadedAssetUrls.TryGetValue(url, out previousReference))
            {
                ...
                if (previousReference.Next != null) previousReference.Next.Prev = reference;
